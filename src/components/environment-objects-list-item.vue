...
<div class="environment-objects-list-item">
    <div class="environment-objects-list-item-title"  :class="[object.type, {selected: object == $root.selectedObject}]" v-on:click="select">{{ object.name }}</div>
    <div class="environment-objects-list-item-children" v-if="object.children.length > 0">
        <CEnvironmentObjectsListItem v-for="child in object.children" :key="child.name" :object="child" />
    </div>
</div>
</template>
